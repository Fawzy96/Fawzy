...
public class Notification_Activity extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < member_names.length; i++) {
            Row_Item item = new Row_Item(member_names[i],
                    profile_pics.getResourceId(i, -1), statues[i]);
            rowItems.add(item);
        }

        mylistview = (ListView) findViewById(R.id.list_v);
